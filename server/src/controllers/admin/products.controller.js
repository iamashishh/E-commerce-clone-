...
module.exports.handleImageUplaod = async (req,res) => {
      try {
  ...
  try {
    ...
    if (!findProduct)
      return res.status(404).json({
        success: false,
        message: "Product not found in editing",
      });
    ...
    res.status(500).json({
      success: false,
      message: "Error occured in editing product",
    });
  }
};
...
module.exports.deleteProduct = async (req, res) => {
  try {
    ...
    if (!product)
      return res.status(404).json({
        success: false,
        message: "Product not found in deleting",
      });
